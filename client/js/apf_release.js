...
this.isWin=(os=="win");this.isMac=(os=="mac");this.isLinux=(os=="linux");},setCompatFlags:function(){this.TAGNAME=apf.isIE?"baseName":"localName";
...
this.hasConditionCompilation=apf.isIE;this.supportOverflowComponent=apf.isIE;this.hasFlexibleBox=apf.versionGecko>2||apf.webkitRev>2;
...
};apf.setZeroTimeout=(apf.isIE&&apf.isIE<9)||apf.isO3||apf.isAIR?setTimeout:(function(){var timeouts=[];
...
}}if(this.$loadAml){this.$loadAml(this.$aml);}};this.addEventListener("DOMNodeInsertedIntoDocument",f);
this.addEventListener("$skinchange",f);var f;this.addEventListener("$event.resize",f=function(c){apf.layout.setRules(this.$ext,"resize","var o = apf.all["+this.$uniqueId+"];            if (o) o.dispatchEvent('resize');",true);
...
};this.getSortSettings=function(){return this.$sort.get();};this.getTraverseNodes=function(xmlNode){if(this.$sort){var nodes=apf.getArrayFromNodelist((xmlNode||this.xmlRoot).selectNodes(this.each));
...
};this.isDragAllowed=function(x,data){if(this.disabled||!x||!x.length||!x[0]){return false;
}if(this.drag||this.dragcopy){if(data){data.merge(x);}return true;}var d,ruleList=[],j=0,l=x.length;
for(;j<l;j++){d=this.$getDataNode("drag",x[j],null,ruleList);if(!d){return false;
}if(data){data.push(d);}}return ruleList.length?ruleList:false;};this.isDropAllowed=function(x,target){if(this.disabled||!x||!x.length||!target){return false;
}var data,tgt,hasDropRule=this.$attrBindings&&this.$attrBindings.drop;if(this.drop&&(!hasDropRule||hasDropRule.value=="true")){this.$setDynamicProperty("drop",this.hasFeature(apf.__MULTISELECT__)?"["+this.each+"]":"[node()]");
...
}}}if(!options.insertPoint){options.insertPoint=this.data;}if(typeof options.copyAttributes=="undefined"){options.copyAttributes=true;
}var newNode=apf.mergeXml(xmlNode,options.insertPoint,options);apf.xmldb.applyChanges("insert",options.insertPoint);
return xmlNode;};this.convertXml=function(type){if(!type){return this.data.xml;
...
this.$ext=this.$getExternal(null,null,function(oExt){var mask=this.getAttribute("mask");
...
});}apf.cgi=function(){this.supportMulticall=false;this.namedArguments=true;this.unserialize=function(str){return str;
};this.getSingleCall=function(name,args,obj){obj.push(args);};this.createMessage=function(functionName,args){var prop,vars=[];
function recur(o,stack){if(o&&o.dataType==apf.ARRAY){for(var j=0;j<o.length;j++){recur(o[j],stack+"%5B"+j+"%5D");
}}else{if(o&&typeof o=="object"){if(o.nodeType){try{var s=o.outerHTML||o.serialize&&o.serialize()||apf.getCleanCopy(o).xml;
